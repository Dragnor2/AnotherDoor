:[0]
b [20]

> gml_Script_InputVibrateADSR (locals=0, argc=9)
:[1]
pushbltn.v builtin.argument7
pushbltn.v builtin.undefined
cmp.v.v EQ
bf [3]

:[2]
pushi.e 0
pop.v.i builtin.argument7

:[3]
pushbltn.v builtin.argument8
pushbltn.v builtin.undefined
cmp.v.v EQ
bf [5]

:[4]
pushi.e 0
pop.v.b builtin.argument8

:[5]
isstaticok.e
bt [7]

:[6]
setstatic.e
call.i gml_Script___InputVibrateSystem(argc=0)
pushi.e -9
push.v [stacktop]self.__playerArray
pop.v.v static._playerArray

:[7]
push.v arg.argument7
call.i is_numeric(argc=1)
conv.v.b
not.b
bf [9]

:[8]
push.s "\")"@194
conv.s.v
push.v arg.argument7
call.i typeof(argc=1)
push.s "Player index must be a number (typeof = \""@6308
conv.s.v
call.i gml_Script___InputError(argc=3)
popz.v

:[9]
push.v arg.argument7
pushi.e 4
cmp.i.v GTE
bf [11]

:[10]
push.s ")"@372
conv.s.v
pushi.e 4
conv.i.v
push.s " too large. Must be less than config `INPUT_MAX_PLAYERS` ("@6309
conv.s.v
push.v arg.argument7
push.s "Player index "@6310
conv.s.v
call.i gml_Script___InputError(argc=5)
popz.v

:[11]
push.v arg.argument7
pushi.e 0
cmp.i.v LT
bf [13]

:[12]
push.s " less than zero"@6311
conv.s.v
push.v arg.argument7
push.s "Player index "@6310
conv.s.v
call.i gml_Script___InputError(argc=3)
popz.v

:[13]
pushi.e 1
conv.i.v
pushi.e 0
conv.i.v
push.v arg.argument0
call.i clamp(argc=3)
pop.v.v arg.argument0
pushi.e 1
conv.i.v
pushi.e 0
conv.i.v
push.v arg.argument1
call.i clamp(argc=3)
pop.v.v arg.argument1
pushi.e 1
conv.i.v
pushi.e -1
conv.i.v
push.v arg.argument2
call.i clamp(argc=3)
pop.v.v arg.argument2
pushi.e 0
conv.i.v
push.v arg.argument3
call.i max(argc=2)
pop.v.v arg.argument3
pushi.e 0
conv.i.v
push.v arg.argument4
call.i max(argc=2)
pop.v.v arg.argument4
pushi.e 0
conv.i.v
push.v arg.argument5
call.i max(argc=2)
pop.v.v arg.argument5
pushi.e 0
conv.i.v
push.v arg.argument6
call.i max(argc=2)
pop.v.v arg.argument6
pushi.e -16
push.v arg.argument7
conv.v.i
push.v [array]self._playerArray
pushi.e -9
pushenv [19]

:[14]
push.v builtin.__paused
conv.v.b
not.b
bt [16]

:[15]
push.v arg.argument8
conv.v.b
b [17]

:[16]
push.e 1

:[17]
bf [19]

:[18]
push.v arg.argument8
push.v arg.argument6
push.v arg.argument5
push.v arg.argument4
push.v arg.argument3
push.v arg.argument2
push.v arg.argument1
push.v arg.argument0
push.i [function]gml_Script___InputVibrateClassADSR
conv.i.v
call.i @@NewGMLObject@@(argc=9)
push.v builtin.__eventArray
call.i array_push(argc=2)
popz.v

:[19]
popenv [14]
exit.i

:[20]
push.i [function]gml_Script_InputVibrateADSR
conv.i.v
pushi.e -1
conv.i.v
call.i method(argc=2)
dup.v 0
pop.v.v self.InputVibrateADSR
popz.v

:[end]